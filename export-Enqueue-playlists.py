...
def main():
	nq = EnqueueWrapper()
	nq.readDB()
	print pprint.pprint(nq.playlists_info)
	nq.writeFiles()
...
class EnqueueWrapper():
	def __init__(self,
		...
		self.erase_files_not_needed  = erase_files_not_needed
	...
	def writeFiles(self):
		...
		for playlist_name in self.playlists_info.keys():
		...
		# now remove files self.out_dir no longer used
		for fn in existing_files:
			...
			if self.erase_files_not_needed:
				print "TODO: Deleting file no longer needed: " + self.out_dir + fn

		print "Now there is %s MB needed, and copied %s MB." % (self.getMB(needed_bytes), self.getMB(copied_bytes))
		if self.erase_files_not_needed:
