...
''' 
	...
	def writeM3uFiles(self):
		...
		for playlist_name in self.playlists_to_copy:
			...
			m3u_text = u"#EXTM3U - \"%s\" playlist generated by chad from Enqueue on %s \n\n" \
			           % (playlist_name, now)
			for song in songs:
	...
	def copySongs(self):
		...
		for db_path in self.songs2copy:
			...
			dest_path_fn = dest_path_fn.encode('latin-1')
			self.files_needed.append(dest_fn)
			if dest_fn in self.existing_dest_files:
				if self.verbose:
					print "Not copying, already exists: " + dest_path_fn
				continue
			...
			if self.verbose:
				...
				print "    Copying file to  : " + dest_path_fn + "\n"
	...
	def dbPath2DestFn(self, path):
		...
		if not p.scheme == 'file':
			self.number_errors += 1
			print "\nWARNING: FIX dbPath2DestFn: "+ path
		#path_fn = urllib.url2pathname(p.path)  # converts to unicode, does not work
		path_fn_uni    = urllib.url2pathname(p.path)   # works with python ?
		path_fn_latin1 = path_fn_uni.encode('latin-1') # works with print, files
